...
public:
	 ...
	 */
	size_t CountAlive() { return EnumerateAlive().Count(); }
...
protected:
	...
	void PopList()
	{
		...
		if (!m_cont.listSet[listIndex]) {
			...
			// Now its safe to cleanup this list's memory.
			Gods98::Mem_Free(m_cont.listSet[listIndex]);
